...
export const dispatch = action.bound((action, stackNavState = true) => {
  store.nav = RootNavigator.router.getStateForAction(action, stackNavState ? store.nav : null)
  return store.nav
})
...
export const AppNavigation = observer(class Navigation extends Component {
  render() {
    return (
      <RootNavigator navigation={addNavigationHelpers({ dispatch, state: store.nav })} />
    )
  }
})
